...
def expectation_value_qndm(lambda1, pars, num_qub, num_l, val_g, shift, ent_gate, newspop, shots):
    ...
    gradient_component = asin(2 * p1 - 1) / (lambda1)
...
def qndm_derivative(args):
    ...
    # derivative in the direction e_(shift_position)
    # TODO: add sinus function, lambda can change in the main so the function sinus is not always 1
