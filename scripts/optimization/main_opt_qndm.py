...
# Read the variable and add into sys.path
...
n_iter     = int(sys.argv[10])   # number of optimization iterations
...
print_run_card(output_dir, n_qubits, n_layers, parameters, val_g, shots, newspop,  ent_gate, lr, method, lambda1, shift)
...
for i in tqdm(range(n_iter), desc = 'Iteration : ', leave=True):
    ...
    # qndm gradient 
    grad_qndm = qndm_gradient(lambda1, parameters, newspop, n_qubits, n_layers, ent_gate, shift, shots, val_g)

    # gradient descent optimization step 
    parameters = parameters - lr * grad_qndm
