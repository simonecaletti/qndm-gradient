...
def get_qndm_error(pars, G_real_qndm, lambda1, shots, shift=np.pi/2):
    error = np.zeros(len(pars))  # Usa len(pars) per la lunghezza dell'array
    
    for i in range(len(pars)):
        error[i] = (1 / (2 * np.sqrt(shots))) * \
                   (1 / (np.sqrt(1 - (np.sin(lambda1 * np.sin(shift) * G_real_qndm[i])) ** 2))) * \
                   (1 / (lambda1 * np.sin(shift)))
